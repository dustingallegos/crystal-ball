...
(function () {
     ...
     * @returns {string}
     */
    function getConstituentSearch(request, response) {
        Sky.getConstituentSearch(request, request.params.searchText, function (results) {
            console.log('getConstituentSearch() response:\n' + JSON.stringify(results, null, '\t'));
            response.send(results);
        });
    }
